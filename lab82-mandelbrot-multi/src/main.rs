...
fn main() {
    ...
    // Calculate all pixels in parallel using Rayon
    let pixels: Vec<(u32, u32, Rgb<u8>)> =
        ...
        .flat_map(|y| {
            (0..image_width).into_par_iter().map(move |x| {
                ...
                // Calculate Mandelbrot set value for this point
                ...
                let pixel = if iteration == max_iterations {
                    Rgb([0, 0, 0]) // Black for points in the set
                } else {
                    // Use HSV color space for smooth coloring
                    let hue = (iteration as f64 / max_iterations as f64) * 360.0;
